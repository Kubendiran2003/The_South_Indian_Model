...
export async function handler(event, context) {
  ...
  try {
    const { fullName, email, mobile, company, requestType, budget, preferredDate, message } = JSON.parse(event.body);
    ...
    if (error) {
      ...
      return {
        statusCode: 400,
        body: JSON.stringify({ error: error.message || 'Failed to send email via Resend' }),
      };
    }
    ...
    return {
      statusCode: 500,
      body: JSON.stringify({ error: err.message || 'Internal server error occurred' }),
    };
  }
}
